...
class TitleState extends MusicBeatState
{
	...
	public static var isEvil:Bool = false;
	...
	function startIntro()
	{
		...
		grad = new FlxSprite().loadGraphic(Paths.image('gradient'));
		grad.alpha = 0;
		add(grad);

		evilBoyfriend = new FlxSprite().loadGraphic(Paths.image('EVIL'));
		evilBoyfriend.x -= 600;
		add(evilBoyfriend);

		logoBl = new FlxSprite(450);
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 15:
					addMoreText('EVIL');
				case 16:
					addMoreText('BF'); // credTextShit.text += '\nFunkin';
